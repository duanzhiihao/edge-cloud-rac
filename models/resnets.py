...
class BottleneckResNetLayerWithIGDN(CompressionModel):
    def __init__(self, zdim=32, feature_dim=256):
        super().__init__(entropy_bottleneck_channels=zdim)
        self.encoder = nn.Sequential(
            nn.Conv2d(feature_dim, zdim * 2, kernel_size=3, stride=1, padding=1, bias=False),
            GDN1(zdim * 2),
            nn.Conv2d(zdim * 2, zdim, kernel_size=3, stride=1, padding=1, bias=True)
        )
        self.decoder = nn.Sequential(
            nn.Conv2d(zdim, feature_dim*2, kernel_size=3, stride=1, padding=1, bias=False),
            GDN1(feature_dim * 2, inverse=True),
            nn.Conv2d(feature_dim*2, feature_dim, kernel_size=3, stride=1, padding=1, bias=True),
        )
...
class BottleneckResNet(nn.Module):
    def __init__(self, zdim=32, num_classes=1000, bpp_lmb=0.02, teacher=True):
        super().__init__()
    ...
    def forward(self, im, y):
        ...
        x = self._model.conv1(im)
        x = self._model.bn1(x)
        x = self._model.relu(x)
        x = self._model.maxpool(x)
        x1 = self._model.layer1(x)
        x1_hat, p_z = self.bottleneck_layer(x1)
        x2 = self._model.layer2(x1_hat)
        x3 = self._model.layer3(x2)
        x4 = self._model.layer4(x3)
        feature = self._model.avgpool(x4)
        feature = torch.flatten(feature, 1)
        logits_hat = self._model.fc(feature)

        probs_teach, features_teach = self.forward_teacher(im)

        # bit rate loss
        bppix = -1.0 * torch.log2(p_z).mean(0).sum() / float(imH * imW)
        # feature reconstruction loss
        l_rec = tnf.mse_loss(x1_hat, x1, reduction='mean')
        ...
        loss = lmb_cls * (0.5*l_ce + 0.5*l_kd) + lmb_trs * sum(l_trs) + lmb_bpp * bppix + l_rec

        stats = OrderedDict()
        stats['loss'] = loss
        stats['bppix'] = bppix.item()
        stats['rec'] = l_rec.item()
        stats['CE'] = l_ce.item()
        stats['KD'] = l_kd.item()
    ...
    def self_evaluate(self, x, y):
        ...
        if self.compress_mode:
            ...
            x1_hat = self.bottleneck_layer.decompress(compressed_obj)
        else:
            x1_hat, p_z = self.bottleneck_layer(x1)
        x2 = self._model.layer2(x1_hat)
        x3 = self._model.layer3(x2)
...
@register_model
def resnet_c1(num_classes=1000, bpp_lmb=0.04, teacher=True):
    model = BottleneckResNet(32, num_classes=num_classes, bpp_lmb=bpp_lmb, teacher=teacher)
    return model
